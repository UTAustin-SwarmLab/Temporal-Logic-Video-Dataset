...
class COCOImageLoader(TLVImageLoader):
    ...
    def __init__(
        ...
        self._original_class_labels = [
            cat["name"].replace(" ", "_") for cat in self._coco.cats.values()
        ]
        self.class_labels = [
            label.replace(" ", "_")
    ...
    def process_data(self) -> TLVRawImage:
        """Process raw COCO data to TLVRawImage Data Class."""
        img_ids = self._coco.getImgIds()
        images = []
        class_labels = []
        # class_labels:
        ...
        return TLVRawImage(
            unique_labels=self.class_labels, labels=class_labels, images=images
        )
