...
class NuScenesImageLoader:
    ...
    def convert_to_tlv_dataset(self, raw_data: any):
        benchmark_frame = TLVDataset(
            ...
            ltl_formula="",
            proposition=[],
            number_of_frame=len(raw_data["images_of_frame"]),
            frames_of_interest=[],
            labels_of_frames=raw_data["labels_of_frame"],
            images_of_frames=raw_data["images_of_frame"],
        )

    def loading_data(self, generate_func: callable = None):
        scene_data = {}
        for scene in self._nuscene:
        ...
        try:
            ...
            for unique_prop in unique_prpos:
                if self.evaluate_unique_prop(
                    unique_prop, benchmark_frame.labels_of_frames[:5]
                ):
                    (
                        ltl_formula,
                        new_prop_set,
    ...
    def update_and_save_benchmark_frame(
        ...
        assert len(benchmark_frame_.images_of_frames) == len(
            benchmark_frame_.images_of_frames
        )
    ...
    def all_labels(self) -> list:
        return list(MAPPER_METADATA.keys())
