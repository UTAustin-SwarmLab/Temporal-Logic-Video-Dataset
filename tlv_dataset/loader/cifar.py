...
class Cifar10ImageLoader(TLVImageLoader):
    ...
    def __init__(
        ...
        self.class_labels = [
            label
            for label in self._original_class_labels
            if label in LABEL_OF_INTEREST
        ]
        self.data: TLVRawImage = self.process_data(
            raw_data=self.load_data(
                data_path=self._cifar_dir_path / self._batch_id
            )
        )

    def process_data(self, raw_data) -> any:
        """Process raw data to TLVRawImage Data Class."""
        class_labels = list(
            map(lambda x: [self._original_class_labels[x]], raw_data[b"labels"])
        )
        data = raw_data[b"data"].reshape(-1, 3, 32, 32).transpose(0, 2, 3, 1)
        plt.imshow(data[4])
        plt.savefig("test_1.png")
        return TLVRawImage(
            unique_labels=self.class_labels, labels=class_labels, images=data
        )
...
class Cifar100ImageLoader(TLVImageLoader):
    """Load CIFAR 10 image data from file."""
    ...
    def __init__(
        ...
        self.data: TLVRawImage = self.process_data(raw_data=self._raw_data)
    ...
    def process_data(self, raw_data) -> any:
        ...
        class_labels = list(
            map(
                lambda x: [self._original_class_labels[x]],
                raw_data[b"fine_labels"],
            )
        )
        # class_labels:
        # image_idx_1: "label"
