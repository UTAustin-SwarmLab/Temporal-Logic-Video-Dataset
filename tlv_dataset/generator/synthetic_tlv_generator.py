...
class SyntheticTLVGenerator(DataGenerator):
    ...
    def generate(
        self,
        initial_number_of_frame: int = 25,
    ...
    def until_time_delta_ltl_function(
        ...
        prop1_range = self.generate_unique_random_indices_within_range(
            start=0, end=prop2_placeholder - 1
        )
        # labels_of_frame[0] = proposition_1
    ...
    def ltl_function(
        ...
        random_frame_idx_selection = sorted(
            list(
                set(
                    [
                        random.randint(0, number_of_frame - 3)
                        for _ in range(int(number_of_frame / 5))
                    ]
                )
            )
        ...
        if proposition_2 is not None:
            ...
            if temporal_property == "U":
                    ...
                    for i, temp_label in enumerate(temp_frames_of_interest):
                        if temp_label not in post_u_label_idx:
                            if temp_label > max(post_u_label_idx):
                                ...
                                temp_frames_of_interest.pop(
                                    temp_frames_of_interest.index(temp_label)
                                )
                ...
                assert (
                    conditional_property is not None
                ), "conditional_property must be not None"
            ...
            if temporal_property == "F":
                frame_index = [
                    random.randint(0, number_of_frame - 1)
                    for _ in range(int(number_of_frame / 5))
                ]
                ...
                frame_index = [
                    random.randint(0, number_of_frame - 1)
                    for _ in range(int(number_of_frame / 10))
                ]
                for idx in list(set(frame_index)):
                ...
                if conditional_property == "!":
                    temp_frames_of_interest = [
                        x
                        for x in range(len(labels_of_frame))
